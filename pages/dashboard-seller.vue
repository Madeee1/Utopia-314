<template>
 <div class="flex flex-col items-center">
    ...
    <button class="btn logout-button absolute top-4 left-4" @click="logout">Log Out</button>
    <button class="btn absolute top-4 right-4" @click="moveToProfile">Update Profile</button>
    <div class="grid grid-cols-3 gap-4">
      <div class="card" v-for="listing in listings" :key="listing.id">
        ...
        <b><p class="card-text">Agent Name : {{ listing.agent }}</p></b>
        <p class="card-text">Views: {{ listing.views }}</p>
         <p class="card-text">Shortlisted: {{ listing.shortlistedCount }}</p> <!-- Displaying shortlisted count -->
        <button class="btn" @click="showReviewModal(listing.id)">Rate and Review my agent</button>
      </div>
    </div>
    <div v-if="reviewModalVisible" class="overlay">
      <div class="modal">
        ...
        <input type="number" v-model="reviewRating" min="1" max="5" class="input mb-4" />
        <textarea v-model="reviewText" placeholder="Write a review..." class="input mb-4"></textarea>
...
async function fetchListings() {
  const response = await $fetch(`/api/listings?sellerId=${sessionStorage.getItem("userId")}`);
  listings.value = response;
}

async function searchListings() {
  if (searchQuery.value.trim()) {
    const response = await $fetch(`/api/listings/search?query=${encodeURIComponent(searchQuery.value)}&sellerId=${sessionStorage.getItem("userId")}`);
    listings.value = response;
  } else {
    await fetchListings();
  }
...
const listings = ref([
  ...
  },
  {
    ...
    description: "A spacious and centrally located condo, perfect for families.",
    agent : "Majeed",
    price: 450000,
    views: 325,
    shortlistedCount: 15
